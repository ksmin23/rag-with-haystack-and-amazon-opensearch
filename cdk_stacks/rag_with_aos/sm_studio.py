...
class SageMakerStudioStack(Stack):
  ...
  def __init__(self, scope: Construct, construct_id: str, vpc, ops_client_sg_id, **kwargs) -> None:
    ...
    sg_sagemaker_domain = aws_ec2.SecurityGroup(self, 'SageMakerDomainSG',
      ...
      security_group_name='sagemaker-studio-domain-sg'
    )
    ...
    SAGEMAKER_DOMAIN_NAME = self.node.try_get_context('sagemaker_domain_name')
    ...
    default_user_settings = aws_sagemaker.CfnUserProfile.UserSettingsProperty(
      ...
      security_groups=[sg_sagemaker_domain.security_group_id, ops_client_sg_id]
    )
